...
const WorkflowView: React.FC = () => {
  ...
  const templateCapabilityTags = useMemo(() => {
    ...
    if (selectedTemplate.sourceType) {
      tags.push(selectedTemplate.sourceType === "builtin" ? "内置模板" : "数据库模板");
    }
    return tags;
  }, [selectedTemplate]);
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      ...
      setTemplates((prev) => prev.map((template) => (template.code === saved.code ? saved : template)));
        ...
        {!currentWorkflow ? (
          ...
          <>
            <WorkflowGraphCard
              ...
              steps={steps}
              templateCapabilityTags={templateCapabilityTags}
              templateEditorValue={templateEditorValue}
              templateSaving={templateSaving}
              templates={templates}
              onResetTemplateEditor={handleResetTemplateEditor}
