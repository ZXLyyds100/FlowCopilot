...
@Slf4j
@Configuration
@EnableConfigurationProperties(FlowCopilotLlmProperties.class)
public class MultiChatModelConfig {

    @Bean
    public ChatModelRegistry chatModelRegistry(FlowCopilotLlmProperties properties) {
    ...
    private void registerDeepSeek(FlowCopilotLlmProperties.ProviderProperties config, Map<String, ChatModel> chatModels) {
        if (!config.isUsable()) {
    ...
    private void registerZhipu(FlowCopilotLlmProperties.ProviderProperties config, Map<String, ChatModel> chatModels) {
        if (!config.isUsable()) {
    ...
    private void logProviderSkip(String modelName, FlowCopilotLlmProperties.ProviderProperties config) {
        if (config.isEnabled()) {
