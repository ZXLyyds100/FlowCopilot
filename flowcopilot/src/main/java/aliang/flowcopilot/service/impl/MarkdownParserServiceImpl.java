 ...
 * Markdown 解析服务实现。
 * <p>
 * 基于 Flexmark 解析 Markdown 文本，并按顶层标题划分章节，
 * 为后续文档切块和向量化提供结构化输入。
 */
@Service
@Slf4j
public class MarkdownParserServiceImpl implements MarkdownParserService {
     ...
     * 从文档 AST 中提取标题和对应内容。
     * <p>
     * 当前实现只遍历顶层节点；遇到任意标题时，会把它到下一个标题之间的内容
     * 作为当前章节正文。
     *
     * @param document Markdown AST 根节点
     * @param sections 输出章节集合
     */
     ...
     * 提取节点内容。
     * <p>
     * 当节点为表格时，尽量保留原始 Markdown 格式；否则退化为纯文本提取。
     *
    ...
    /**
     * 从原始文档中提取表格的 Markdown 原文。
     *
     * @param tableNode 表格节点
     * @return 表格 Markdown 内容
     */
