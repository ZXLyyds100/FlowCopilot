...
> 截图占位 02：建议插入“Agent 聊天页面”截图，突出左侧 Agent/会话列表、右侧消息区、流式回复效果。
...
> 截图占位 03：建议插入“知识库管理 / 文档上传 / RAG 召回结果”截图，展示文档列表和知识引用。
...
> 截图占位 04：建议插入“工作流创建 + Graph 图谱”截图，展示任务输入、模板选择和节点运行状态。

> 截图占位 05：建议插入“人工审批面板”截图，展示待审批记录、审批意见、通过和驳回按钮。
...
> 截图占位 06：建议插入“Trace / Checkpoint / Observability 面板”截图，展示节点耗时、Span 树、外部 Trace 状态。
